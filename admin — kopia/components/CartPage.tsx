...
export default function CartPage() {
  ...
  const handleApplyPromo = useCallback((e: React.FormEvent) => {
    e.preventDefault();
      ...
      <div className="cart-page">
        <div className="cart-page-container">
          ...
          <div className="empty-cart">
            ...
            <h2 className="empty-cart-title">Twój koszyk jest pusty</h2>
        ...
        <div className="cart-content">
                    ...
                    <div className="cart-item-details">
                      ...
                      {(item.selectedSize || item.selectedColor) && (
                        <div className="cart-item-options">
                          {item.selectedSize && (
                            <span className="cart-item-option">Rozmiar: {item.selectedSize}</span>
                          )}
                          {item.selectedColor && (
                            <span className="cart-item-option">
                              Kolor: {item.product.colors?.find(c => c.value === item.selectedColor)?.name || item.selectedColor}
                            </span>
                          )}
                        </div>
                      )}
          ...
          <div className="cart-summary">
            <div className="cart-summary-card">
              <h2 className="cart-summary-title">Podsumowanie zamówienia</h2>
              
              <div className="cart-summary-row">
                <span className="cart-summary-label">Suma częściowa</span>
                <span className="cart-summary-value">{formattedSubtotal} zł</span>
              </div>

              {appliedDiscount > 0 && (
                <div className="cart-summary-row">
                  <span className="cart-summary-label">Zniżka (-{appliedDiscount}%)</span>
                  <span className="cart-summary-value discount">-{formattedDiscount} zł</span>
                </div>
              )}

              <div className="cart-summary-row">
                <span className="cart-summary-label">Koszt dostawy</span>
                <span className="cart-summary-value">{deliveryFee > 0 ? `${formattedDeliveryFee} zł` : 'Gratis'}</span>
              </div>

              <div className="cart-summary-divider"></div>

              <div className="cart-summary-row cart-summary-total">
                <span className="cart-summary-label">Razem</span>
                <span className="cart-summary-value">{formattedTotal} zł</span>
              </div>
