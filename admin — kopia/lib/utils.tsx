...
/**
 * Konwertuje cenę z formatu string (np. "89,00") na liczbę
 */
 ...
 */
export const renderStars = (rating: number, size: number = 20): React.JSX.Element => {
  ...
  return (
    <div className="product-rating-stars">
      {Array(fullStars).fill(0).map((_, i) => (
        <svg key={`full-${i}`} className="star star-full" viewBox="0 0 24 24" width={size} height={size}>
          <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" fill="currentColor"/>
        </svg>
      ))}
      {hasHalfStar && (
        <svg className="star star-half" viewBox="0 0 24 24" width={size} height={size}>
          <defs>
      ...
      {Array(emptyStars).fill(0).map((_, i) => (
        <svg key={`empty-${i}`} className="star star-empty" viewBox="0 0 24 24" width={size} height={size}>
