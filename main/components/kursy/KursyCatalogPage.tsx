...
import CoursesFiltersSidebar, { CourseFiltersState } from "./CoursesFiltersSidebar";

function getCourseCategoryNames(course: Courses): string[] {
    const kats = course.kategoria;
    if (!kats || !Array.isArray(kats)) return [];
    return kats
        .map((c) => (typeof c === "string" ? c : (c as Categories).nazwa))
        .filter(Boolean);
}
...
export default function KursyCatalogPage() {
        ...
        <div className="min-h-screen bg-white pt-[120px] mt-5 pb-16 px-4 sm:px-6 lg:px-8">
            <div className="max-w-[1400px] mx-auto">
                <div className="flex flex-wrap items-center gap-2 text-sm text-gray-500 mb-4">
                    ...
                    <span>&gt;</span>
                    <span className="text-gray-900 bg-[#D2B79B1f] font-bold text-[15px] px-2 py-1 rounded-md">
                        Szkolenia
                    </span>
                </div>
                <div className="flex flex-row justify-between items-center gap-6 my-6 text-sm text-gray-600">
                    ...
                    <div className="flex flex-row items-center gap-2">
                        <span>
                            Wyników: {startIndex + 1}-{Math.min(endIndex, totalItems)} z {totalItems}{" "}
                            szkoleń.
                        </span>
                        ...
                        <select
                            ...
                            className="rounded-md border border-gray-300 bg-white px-2.5 py-1.5 text-sm text-gray-700 focus:border-[#D2B79B] focus:ring-1 focus:ring-[#D2B79B] align-middle">
                            ...
                            {displayedCourses.length === 0 ? (
                                <div className="col-span-full py-16 px-6 text-center rounded-2xl bg-gray-50 border border-gray-200">
                                    ...
                                    <p className="text-gray-600 max-w-md mx-auto">
                                        Wkrótce dodamy nowe szkolenia lub zmień filtry.
                                    </p>
                                </div>
                            ) : (
                                        ...
                                        if (
                                            ...
                                            return (
                                                <button
                                                    key={pageNum}
                                                    type="button"
                                                    className={`min-w-[40px] px-3 py-2 rounded-lg border transition-colors ${currentPage === pageNum ? "bg-[#D2B79B] text-black border-[#D2B79B]" : "border-gray-300 hover:bg-gray-100"}`}
