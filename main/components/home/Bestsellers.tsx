    ...
    useEffect(() => {
        ...
        async function load() {
            if (data?.status === 200 && data?.products?.length) {
                if (!cancelled) {
                    setProducts(
                        [...data.products].sort((a, b) => b.ocena - a.ocena).slice(0, 4),
                    );
                }
                return;
            }
            ...
            try {
                ...
                const list: Products[] = Array.isArray(json.products) ? json.products : [];
                if (!cancelled) {
                    setProducts([...list].sort((a, b) => (b.ocena ?? 0) - (a.ocena ?? 0)).slice(0, 4));
                }
            } catch {
    ...
    return (
        <section className="w-full py-16 px-4 sm:px-6 lg:px-8 bg-white/30" id="bestsellers-section">
            <div className="max-w-[1400px] mx-auto">
                ...
                {!products?.length && (
                    <div className="text-center py-8 text-gray-600">Brak produktów do wyświetlenia.</div>
                )}
