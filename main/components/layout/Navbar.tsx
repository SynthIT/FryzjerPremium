...
export function Navbar({ onMenuClick }: { onMenuClick?: () => void }) {
    ...
    return (
        <header className="fixed top-0 left-0 right-0 z-50 border-b bg-background/95 backdrop-blur-md supports-backdrop-filter:bg-background/95 shadow-sm admin-navbar h-16">
            <div className="flex h-full items-center justify-between px-4 sm:px-6 md:px-8">
                ...
                <div className="flex items-center gap-1">
                    <button
                        ...
                        onClick={toggleTheme}
                        className="rounded p-2 transition-colors hover:bg-accent focus:outline-none focus:ring-2 focus:ring-ring active:scale-95">
                        {mounted && theme === "dark" ? (
                    ...
                    <DropdownMenu.Root>
                        <DropdownMenu.Trigger asChild>
                            <button
                                aria-label="Konto"
                                className="ml-1 inline-flex items-center rounded-full border p-1 transition-all hover:bg-accent focus:outline-none focus:ring-2 focus:ring-ring data-[state=open]:scale-95">
                        ...
                        <DropdownMenu.Portal>
                            <DropdownMenu.Content
                                sideOffset={8}
                                className="min-w-[220px] overflow-hidden rounded-md border bg-popover p-1 text-popover-foreground shadow-md will-change-[transform,opacity] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:fade-in-0 data-[state=closed]:fade-out-0 data-[state=open]:zoom-in-95 data-[state=closed]:zoom-out-95">
                                ...
                                <DropdownMenu.Item asChild>
                                    <button
                                        className="flex w-full cursor-pointer select-none items-center gap-2 rounded-sm px-2 py-1.5 text-sm text-red-600 outline-none transition-colors hover:bg-accent"
                                        onClick={() =>
